...
- HTTP/3
 ...
 ### HTTP/1.0、HTTP/1.1和HTTP/2.0是HTTP协议的不同版本，它们之间存在以下区别：
...
### Webpack 3和Webpack 4是Webpack的不同版本，它们之间存在以下区别：

1. 性能优化：
   - Webpack 4引入了持久化缓存（Persistent Caching）的功能，通过将模块标识符映射到持久化的缓存ID，可以减少重新构建时的文件变动检测和重新编译时间。
   - Webpack 4在构建速度和打包结果大小方面进行了优化，采用了更高效的算法和机制，提供了更快的构建速度和更小的文件体积。

2. 模块系统：
   - Webpack 3使用CommonJS作为默认的模块系统。
   - Webpack 4可以支持多种模块系统，包括CommonJS、AMD、ES6模块等，并且推荐使用ES6模块作为默认的模块系统。

3. Tree Shaking优化：
   - Webpack 4在Tree Shaking方面进行了改进，通过静态分析代码，只打包使用到的模块和代码片段，减少了生成的代码体积。

4. Mode模式：
   - Webpack 4引入了Mode模式，可以指定为development、production或none。不同的模式会对构建结果进行不同的优化和调整，如开启压缩、优化代码等。

5. 配置文件变更：
   - Webpack 3使用webpack.config.js作为配置文件的默认名称。
   - Webpack 4支持零配置（Zero Configuration），即在项目中无需显式指定配置文件，Webpack会根据约定和默认值进行配置。
...
### loader 工作流程
- 识别文件类型： 当Webpack处理模块时，会根据文件的扩展名识别文件类型。例如，.js 文件会被认为是JavaScript模块，.css 文件会被认为是样式表模块。

- 匹配Loader规则： 针对不同类型的文件，Webpack会根据配置中的Loader规则进行匹配。Loader规则通常是在Webpack配置文件中进行配置的，用于告诉Webpack在遇到特定类型的文件时应该使用哪个Loader进行处理。

- 加载和转换： 一旦匹配到Loader规则，Webpack会调用相应的Loader对模块文件进行加载和转换。每个Loader可以对模块文件进行不同的转换，例如将ES6代码转换为ES5，将SCSS文件转换为CSS等。

- Loader链： 在Webpack中，可以通过数组配置多个Loader，这些Loader会按照从右到左的顺序依次执行，形成一个Loader链。每个Loader的输出将作为下一个Loader的输入。这样的设计使得可以使用多个Loader协同工作，完成复杂的转换任务。

- 输出： Loader链执行完成后，最终的输出会被传递给Webpack下一个阶段的处理，例如经过Loader处理的JavaScript代码将被传递给下一个阶段的编译器进行编译。
...
### Bridge 和 js2native 的区别
  - "Bridge"（桥接）是一种通信机制，用于在不同的执行环境（例如，H5和宿主应用）之间进行通信和数据交换。它提供了一种接口或方法，使得不同环境中的代码能够相互调用和传递数据。

  - "js2native"（JavaScript到原生）是指在H5页面中使用JavaScript代码调用宿主应用的原生功能或方法。它是一种通过桥接机制实现的方式，可以将H5页面中的JavaScript代码与宿主应用的原生代码进行交互。

  - 实际上，"js2native"可以是基于"bridge"机制的一种具体实现方式。通过建立桥接机制，H5页面可以调用桥接接口发送请求给宿主应用，而宿主应用则通过监听和处理这些请求，执行相应的原生功能。

  - 综上所述，"bridge"和"js2native"之间的关系是，"js2native"是一种基于"bridge"机制实现的H5页面与宿主应用之间的通信方式。"bridge"提供了通信的基础设施，而"js2native"则是其中一种具体的应用场景。

### 虚拟滚动的方式

1. 基于可视区域渲染（Windowing）：

- 在列表容器中创建一个固定高度的可视区域。
- 监听滚动事件，根据滚动位置计算可视区域中的列表项索引范围。
- 只渲染可视区域内的列表项，其余项则保持隐藏或未渲染状态。
- 使用占位符元素（Placeholder）或固定高度的空白元素占据未渲染的列表项的空间，以保持正确的滚动位置和滚动条长度。
- 根据滚动行为动态加载和卸载列表项，以保持可视区域内的列表项数量。

2. 基于动态加载（Infinite Loading）：

- 在列表容器中创建一个固定高度的可视区域。
- 监听滚动事件，当滚动接近可视区域底部时，加载更多的数据。
- 在列表末尾添加新加载的列表项，而不是一次性渲染整个列表。
- 使用占位符元素（Placeholder）或固定高度的空白元素占据未加载的列表项的空间，以保持正确的滚动位置和滚动条长度。
- 根据滚动行为动态加载更多的数据，并在加载完成后更新列表的高度和滚动条的位置。
