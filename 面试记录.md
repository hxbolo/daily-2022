...
2. serve 启动后， 启动 websocket 服务，建立本地服务与浏览器双向通信
...
Pinia 和 Vuex 都是用于状态管理的工具，特别是在 Vue.js 应用中。它们有一些重要的区别：

1. **响应性：**

   - **Vuex：** Vuex 使用 Vue.js 内置的响应性系统来处理状态的变化。它对状态的修改必须是同步的，这意味着当你提交一个 Mutation 时，状态会立即更新，这对于某些场景可能不够灵活。
   - **Pinia：** Pinia 则是建立在 Vue 3 的响应式系统（Composition API）之上的。这意味着它更好地支持异步操作，如在 Action 中进行异步 API 调用，而状态变化仍然会触发视图的更新。

2. **类型安全：**

   - **Vuex：** Vuex 是一个基于对象的库，没有内置的类型检查。开发人员需要小心地确保数据的正确性。
   - **Pinia：** Pinia 使用 TypeScript 来提供类型安全，可以在编译时捕获错误。这意味着开发人员可以更轻松地避免一些常见的错误。

3. **模块化：**

   - **Vuex：** Vuex 使用模块化来组织状态。但在一些情况下，模块化机制可能变得复杂。
   - **Pinia：** Pinia 支持自然的模块化，使得状态管理的组织更加直观。

4. **性能：**
   - **Vuex：** Vuex 在处理大型状态树时可能会引起性能问题，因为它需要遍历整个状态树来检测状态的变化。
   - **Pinia：** Pinia 通过在状态变化时只更新订阅了相关状态的组件来提高性能。

总的来说，Pinia 是一个在 Vue 3 生态系统中更加现代化和高效的状态管理解决方案，特别是对于大型应用或需要处理异步操作的应用。它提供了类型安全、性能优化以及更好的模块化支持。但 Vuex 仍然是 Vue.js 2 项目的流行选项，而且在小型应用中可能更加简单易用。你可以根据你的具体需求来选择使用哪个工具。
...
### 为什么跨域是用配置 proxy 就不会跨域了

你可以将特定路径的请求代理到后端服务的地址

### 原型链为什么要有原型链

### 移动端， pc 端屏幕自适应

在开发中，移动端和 PC 端的屏幕自适应通常需要考虑不同屏幕尺寸和设备的差异。以下是一些常见的方法和技术，可以实现移动端和 PC 端屏幕的自适应：

### 1. 使用响应式设计（Responsive Design）：

- **CSS 媒体查询：** 使用 CSS 媒体查询可以根据不同的屏幕尺寸应用不同的样式。
...
- **弹性布局（Flexbox）和网格布局（Grid）：** 这些布局模型可以帮助更灵活地处理不同屏幕尺寸下的布局。

### 2. 使用相对单位：

- **使用百分比和 em/rem：** 使用相对单位可以使元素相对于其父元素或根元素进行自适应。

### 3. 视口设置：

- **Viewport Meta 标签：** 在 HTML 中通过设置 viewport meta 标签可以控制页面在移动设备上的显示。
...
- **使用 `@media` 查询：** 可以在 CSS 中使用 `@media` 查询根据屏幕宽度应用不同的样式。
...
- **Flexbox：** 适用于一维布局，特别适合移动端的排列。
- **Grid：** 适用于二维布局，提供更灵活的布局选项。

### 6. 使用 rem 单位：

- **使用 rem 单位：** 使用相对于根元素的 rem 单位，可以根据根元素的字体大小调整整个页面的大小。

### 7. JavaScript 动态适应：

- **窗口大小改变事件：** 监听窗口大小改变事件，根据不同的窗口大小动态调整页面布局。
...
- **设备检测：** 可以使用 JavaScript 判断设备类型，然后动态加载不同的样式或布局。

这些方法可以根据实际项目需求进行组合使用，使得页面在不同的设备和屏幕尺寸下都能够有良好的展现和用户体验。

### object.prototype.toString.call()为什么可以判断类型

Object.prototype.toString 方法被特殊处理，返回一个格式为 [object 类型] 的字符串

### 当面试官询问你如何解读 HTTP 请求头和响应头时，你可以简明扼要地解释这两者的作用和一些常见的字段。

### 请求头（Request Headers）:

1. **Host**: 指定服务器的域名和端口号。
2. **User-Agent**: 发起请求的用户代理，通常包含浏览器或应用程序的信息。
3. **Accept**: 表示客户端可以处理的媒体类型。
4. **Authorization**: 包含用于验证用户身份的凭证，例如登录凭据。
5. **Cookie**: 包含客户端向服务器发送的 Cookie 信息，用于身份验证和跟踪用户状态。
6. **Referer**: 表示请求来源的页面 URL。

### 响应头（Response Headers）:

1. **Content-Type**: 指定响应体的媒体类型，例如 text/html 或 application/json。
2. **Content-Length**: 表示响应体的长度。
3. **Date**: 指定响应创建的日期和时间。
4. **Server**: 表示服务器的软件名称和版本。
5. **Set-Cookie**: 在响应中设置 Cookie。
6. **Cache-Control**: 控制缓存的行为，例如是否可以缓存、缓存的有效期等。
7. **Location**: 用于重定向，指定新的资源位置。
8. **Access-Control-Allow-Origin**: 允许的跨域请求的源。

在回答时，可以强调这些头部字段的作用和用途，并举例说明它们如何影响请求和响应的处理。还可以提到一些安全性相关的头部字段，如 Content-Security-Policy、Strict-Transport-Security 等。
